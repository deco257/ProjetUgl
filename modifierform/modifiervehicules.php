...
if(isset($_GET['edit'])){
    $getid=$_GET['edit'];
    $req=$bdd->prepare("SELECT * FROM vehicules WHERE id_vehic=?");
    $req->execute([$getid]);
    $aff=$req->fetch();

    $plaque=$aff['Plaque'];
    $marque=$aff['marque_vehic'];
    $model=$aff['model'];
    $etat=$aff['etat'];
    $carburant=$aff['carburant'];
}
if(isset($_POST['modifier'])){
    $reqt=$bdd->prepare(" UPDATE vehicules SET Plaque=?,marque_vehic=?,model=?,etat=?,carburant=? WHERE id_vehic=?");
    $reqt->execute([$_POST['plaque'],$_POST['marque'],$_POST['modele'],$_POST['etat'],$_POST['carburant'],$getid]);
...
<nav class="navbar navbar-expand-lg navbar-light bg-light  fixed-top">
  ...
  <div class="pull-right">
    ...
    <div class="card">
    <div class="  bg-warning  card-header"> modifier info vehicules </div>
    <div class="card-body">
    <form method="POST" action="">
                        <label class="label control" for="plaque">plaque</label>
                        <input type ="" class="form-control" placeholder="plaque du vehicule"  value="<?php echo $aff['Plaque']?>"id="plaque"   autocomplete="off" name="plaque" required>

                        <label class="label control" for="marque">marque</label>
                        <input type ="" class="form-control" placeholder="moarque du vehicule " value="<?php echo $aff['marque_vehic']?>"id="marque"  autocomplete="off" name="marque" required>
                        <label class="label control" for="model">model</label>
                        <input type ="" class="form-control" placeholder="model du vehicule"  value="<?php echo $aff['model']?>" id="model" autocomplete="off"  name="modele" required>
                        <label class="label control" for="etat">etat</label>
                        <input type ="" class="form-control" placeholder="etat du vehicule"  value="<?php echo $aff['etat']?>"  id="etat"  autocomplete="off"  name="etat" required>
                        <label class="label control" for="carburant">carbuarnt</label>
                        <input type ="" class="form-control" placeholder="plaque du vehicule"  value="<?php echo $aff['carburant']?>"id="carburant"  autocomplete="off" name="carburant" required>
                        
        
                      </div>
                      <div class="modal-footer">
                        <button   type="submit"name="modifier" type="button" class="btn btn-danger">modifier</button>
