...
require('bdd.php');
    $recher=$bdd->prepare("SELECT * FROM infrastructure");
         $recher->execute([]);
         $afficheer=$recher->fetchAll();

              if(isset($_POST['save'])){
              $req=$bdd->prepare("INSERT INTO blocs(num_bloc,id_infra)VALUES(?,?)");
         $req->execute([$_POST['num'],$_POST['infra']]);
}

$req=$bdd->prepare('SELECT * FROM blocs');
$req->execute([]);
$rqt=$req->fetchAll();

//la supression des  informations
if(isset($_GET['delete'])){
  $id=$_GET['delete'];
  $req=$bdd->prepare('DELETE FROM blocs WHERE id_bloc=?');
  $req->execute([$id]);
}
?>
<!DOCTYPE html>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top ">
  ...
  <path d="M6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm-5 6s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zM11 3.5a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1h-4a.5.5 0 0 1-.5-.5zm.5 2.5a.5.5 0 0 0 0 1h4a.5.5 0 0 0 0-1h-4zm2 3a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1h-2zm0 3a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1h-2z"/>
        ...
        <li class="nav-item">
          <a class="nav-link"  href="table.php" class="btn btn-primary">dashord</a>
        </li>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form method="POST" action="">
                          <label class="label-control">num_bloc</label>
                          <input type="" name="num" class="form-control" required>
                          <label class="label-control">infra</label>
                          <select name="infra" class="form-select">
                            <?php foreach($afficheer as $affiche):?>
                              <option value="<?= $affiche['id_infra']?>"><?=$affiche['nom_infra']?></option>
                              <?php endforeach?>
                          </select>
...
<button type="button"  style="margin-left:85%" class=" btn btn-warning mt-9" data-bs-toggle="modal" data-bs-target="#exampleModal">
  ajoutez
</button>
  </div>
  <div class="card-body">
...
<?php foreach($rqt  as $aff): ?>
<tr>
<td> <?= $aff['num_bloc'] ?></td>
<td><?= $aff['id_infra'] ?></td>
<td>
<a  title="editez" href="modifierform/modifierbloc.php?edit=<?= $aff['id_bloc']?>" class=" btn btn-success"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
  <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
</svg></a>
          </td>
          <td>
            <a  title="supprimez" href="bloc.php?delete=<?= $aff['id_bloc']?>" class=" btn btn-danger"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
